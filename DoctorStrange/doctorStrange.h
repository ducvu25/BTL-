...
string ChuyenDoiSangXau(int x);
int chuyenDoiSangSo(string s);
suKien* DocDuLieu(string s, int &n);
...
int handleEvents(string & HP, string & LV, string & EXP, string & TS, string & events) {
	...
	nv[0].HPmax = chuyenDoiSangSo(HP);
	nv[0].HP = nv[0].HPmax;
	nv[0].LV = chuyenDoiSangSo(LV);
	nv[0].EXP = chuyenDoiSangSo(EXP);
	nv[0].TS = chuyenDoiSangSo(TS);
	suKien *sk = new suKien[100];
	sk = DocDuLieu(events, n);
	do{
		i++;
		nv[i] = nv[i-1];
		X:
		switch(sk[i-1].ma){
			...
			case 6:{
				...
				int def = 0;
				int len = s.size();
				for(int i1=0   ; i1<len - 5; i1++)	if(s[i1] == 'a' || s[i1] == 'A')
				for(int i2=i1+1; i2<len - 4; i2++)	if(s[i2] == 't' || s[i2] == 'T')
				for(int i3=i2+1; i3<len - 3; i3++)	if(s[i3] == 't' || s[i3] == 'T')
				for(int i4=i3+1; i4<len - 2; i4++)	if(s[i4] == 'a' || s[i4] == 'A')
				for(int i5=i4+1; i5<len - 1; i5++)	if(s[i5] == 'c' || s[i5] == 'C')
				for(int i6=i5+1; i6<len - 0; i6++)	if(s[i6] == 'k' || s[i6] == 'K')
					att++;
				for(int i1=0   ; i1<len - 6; i1++)	if(s[i1] == 'd' || s[i1] == 'D')
				for(int i2=i1+1; i2<len - 5; i2++)	if(s[i2] == 'e' || s[i2] == 'E')
				for(int i3=i2+1; i3<len - 4; i3++)	if(s[i3] == 'f' || s[i3] == 'F')
				for(int i4=i3+1; i4<len - 3; i4++)	if(s[i4] == 'e' || s[i4] == 'E')
				for(int i5=i4+1; i5<len - 2; i5++)	if(s[i5] == 'n' || s[i5] == 'N')
				for(int i6=i5+1; i6<len - 1; i6++)	if(s[i6] == 's' || s[i6] == 'S')
				for(int i7=i6+1; i7<len - 0; i7++)	if(s[i7] == 'e' || s[i7] == 'E')
					def++;
				att *= 10;
				def *= 10;
				if(att > F){
			...
			case 12:{
				string s1;
				string s2;
				// bien sk bat dau tu 0, i bat dau tu 1
				s1 = sk[i-1].ma1;
				s2 = sk[i-1].ma2;
				if(SK12(s1, s2) == 1){
					nv[i].HP = nv[i].HP*0.9 ; 
...
//	cout << result;
	result = nv[i].HP + nv[i].LV + nv[i].EXP + nv[i].TS;
	ketThuc:
	HP = ChuyenDoiSangXau(nv[i].HP);
	LV = ChuyenDoiSangXau(nv[i].LV);
	EXP = ChuyenDoiSangXau(nv[i].EXP);
	TS = ChuyenDoiSangXau(nv[i].TS);
